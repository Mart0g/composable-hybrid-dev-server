 ...
 */
function evaluateRule(ruleExpression, { host, uri, path, cookies = "" } = {}) {
  const _expr = parseRuleExpression(ruleExpression);

  const args = ["http"]
    .concat(Object.keys(TRANSFORMS))
    .concat(`return ${_expr}`);
  try {
...
if (require.main === module) {
  ...
  const TEST_RULES_3 = ['http.cookies contains "supersecret"'];
  const TEST_RULES_4 = [
  ...
  assert.equal(
    TEST_RULES_2.some((exp) =>
      evaluateRule(exp, {
        host: TEST_HOST,
        cookies: TEST_COOKIES,
        // uri: '/mobify/proxy/foo/bar',
        // path: '/mobify/proxy/foo/bar',
        uri: "/auth/proxy/foo/bar",
        path: "/auth/proxy/foo/bar",
      })
    ),
    true
  );
